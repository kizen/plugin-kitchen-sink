// Kitchen Sink App · View · Workflow Executions View
//
// Opened by workflowDetailView's viewExecutions event script, which forwards
// { automationId, automationName, active, paused, completed } through args (the last three are
// the automation's own summary counts, reused here as filter-pill badges instead of an extra
// fetch). Status quick filters, a paginated executions table, and a click-through to
// executionDetailView per row.
...
const state = stateFromFormData(
  { automationId: [args.automationId], automationName: [args.automationName], active: [args.active], paused: [args.paused], completed: [args.completed] },
);

// Kitchen Sink App · Page · Business Explorer
//
// Ported from a standalone reference SPA (~/kizen-demo-spa) that hit the real Kizen REST API
// directly with manually-entered API Key/Business ID/User ID through its own proxy server. None
// of that applies inside a plugin: the engine already authenticates every `this.get*()` call for
// the installed business, so this page calls the same endpoints straight from the worker.
//
// Four tabs, matching the reference app: Custom Objects, Activities, Activity Objects
// (aggregated — see fetchTabData below), and Agentic Workflows. Each has its own detail view
// opened via showViewInModal (Activities has no per-row detail, matching the reference app).
//
// STATE MODEL: the worker has no persistent state across dispatches (every data-script run is a
// fresh worker), so tab/search/ordering/page are threaded through hidden form inputs on every
// render, not kept in a variable. eventScripts/search.js, paginate.js, and switchTab.js each
// rebuild this same logic from their own copy — the same duplication pattern dashboardBlock's
// refresh.js and myObjectsBlock's refresh.js already use in this repo (no shared module system
// across data-script files).

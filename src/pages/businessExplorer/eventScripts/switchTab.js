// Kitchen Sink App · Page · Business Explorer · switchTab
//
// Fired by a tab-bar <form data-script="switchTab"> submit. Resets search/ordering/page to that
// tab's defaults (matching the reference app's switchTab, which also clears search/sort/page on
// tab change) rather than carrying over the previous tab's state.
...
// Deliberately only reads "tab" — switching tabs resets search/ordering/page to that tab's
// defaults via stateFromFormData's fallback chain, same as the reference app's switchTab().
const state = stateFromFormData({ tab: formData.tab });
